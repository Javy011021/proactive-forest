...
if __name__ == '__main__':
    ...
    # matriz de confusion para proactive
    pf_cmat = confusion_matrix(y_test, pf_predictions)
    # matriz de confusion para random
    rf_cmat = confusion_matrix(y_test, rf_predictions)
    ...
    pf_recall = recall_score(y_test, pf_predictions,
                             average='macro')  # recall PF
    rf_recall = recall_score(y_test, rf_predictions,
                             average='macro')  # recall RF
    ...
    rf_auc = utils.calculate_roc_auc(np.unique(y_train), np.unique(
        y_test), rf, X_test, y_test)  # area bajo la curva RF
